...
export function ModeToggle({ mode, onChange }: ModeToggleProps) {
  ...
  return (
    <button
      ...
      className="relative flex items-center h-auto min-h-[44px] min-w-[44px] sm:h-8 sm:min-h-0 sm:min-w-0 rounded-full bg-slate-700 border border-slate-600 p-0.5 shrink-0"
    ...
    >
      ...
      />
      {/* Edit label (left) */}
      <span className={`relative z-10 px-1.5 sm:px-3 h-full flex items-center gap-1 text-[11px] font-semibold tracking-wide transition-colors duration-150 ${isEdit ? 'text-slate-800' : 'text-slate-400'}`}>
        {/* Pencil icon */}

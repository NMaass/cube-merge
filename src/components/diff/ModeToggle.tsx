...
export function ModeToggle({ mode, onChange }: ModeToggleProps) {
  ...
  return (
    ...
    >
      ...
      />
      {/* Edit label (left) */}
      <span className={`relative z-10 px-3 h-full flex items-center gap-1 text-[11px] font-semibold tracking-wide transition-colors duration-150 ${isEdit ? 'text-slate-800' : 'text-slate-400'}`}>
        {/* Pencil icon */}

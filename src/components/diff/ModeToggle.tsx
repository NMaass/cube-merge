...
export function ModeToggle({ mode, onChange }: ModeToggleProps) {
  ...
  return (
    <button
      ...
      className="touch-target relative flex items-center h-8 sm:h-9 rounded-full bg-slate-700 border border-slate-600 p-0.5 shrink-0"
    ...
    >
      ...
      />
      {/* Edit label (left) */}
      <span className={`relative z-10 px-2.5 sm:px-4 h-full flex items-center gap-1.5 text-xs font-semibold tracking-wide transition-colors duration-150 ${isEdit ? 'text-slate-800' : 'text-slate-400'}`}>
        {/* Pencil icon */}

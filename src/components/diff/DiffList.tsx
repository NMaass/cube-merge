...
import { Change } from '../../types/firestore'
...
interface DiffListProps {
  ...
  changes: Array<Change & { comments: unknown[] }>
}

function SectionHeader({ section, side }: { section: Section; side: 'left' | 'right' }) {
  ...
  return (
    <div
      className="sticky top-[33px] z-10 flex items-center justify-between px-3 py-1"
    ...
    >
      ...
      {count > 0 && (
        <span className={`text-[11px] font-mono ${side === 'left' ? 'text-red-600' : 'text-green-600'}`}>
          {side === 'left' ? '−' : '+'}{count}
        </span>
      )}
    </div>
...
export function DiffList({ sections, imageMap, loadingSet, changes }: DiffListProps) {
  ...
  return (
    <div className="flex flex-col md:flex-row flex-1 min-h-0">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto border-b md:border-b-0 md:border-r border-slate-700">
        <div className="sticky top-0 z-20 flex items-center gap-2 bg-red-950/80 border-b border-red-900/60 px-3 py-2.5 md:py-2">
          ...
          <div key={section.key} id={`section-left-${section.key}`}>
            ...
            <div style={{ backgroundColor: `${COLOR_BG[section.colorCategory]}0e` }}>
              ...
              {section.cardsA.length === 0 && (
                <div className="px-3 py-2 text-xs text-slate-700 border-b border-slate-700/40">—</div>
              )}
            </div>
          </div>
        ))}
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        <div className="sticky top-0 z-20 flex items-center gap-2 bg-green-950/80 border-b border-green-900/60 px-3 py-2.5 md:py-2">
